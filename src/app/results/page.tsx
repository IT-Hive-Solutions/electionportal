...
const constituencyResults: ConstituencyResult[] = [
  {
    id: 'ktm-1', name: 'काठमाडौं-१', province: 'bagmati', district: 'kathmandu', type: 'direct',
    totalRegistered: 85000, totalCast: 52000, countedPercent: 92,
    candidates: [
  ...
  {
    id: 'ktm-2', name: 'काठमाडौं-२', province: 'bagmati', district: 'kathmandu', type: 'direct',
    totalRegistered: 92000, totalCast: 58000, countedPercent: 88,
    candidates: [
  ...
  {
    id: 'morang-1', name: 'मोरङ-१', province: 'koshi', district: 'morang', type: 'direct',
    totalRegistered: 78000, totalCast: 46000, countedPercent: 95,
    candidates: [
  ...
  {
    id: 'jhapa-1', name: 'झापा-१', province: 'koshi', district: 'jhapa', type: 'direct',
    totalRegistered: 82000, totalCast: 50000, countedPercent: 100,
    candidates: [
  ...
  {
    id: 'rupandehi-1', name: 'रुपन्देही-१', province: 'lumbini', district: 'rupandehi', type: 'direct',
    totalRegistered: 88000, totalCast: 55000, countedPercent: 78,
    candidates: [
  ...
  {
    id: 'kaski-1', name: 'कास्की-१', province: 'gandaki', district: 'kaski', type: 'direct',
    totalRegistered: 75000, totalCast: 48000, countedPercent: 100,
    candidates: [
    ...
    return (
      <div className="flex flex-col min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <button onClick={() => setDetailConstituency(null)} className="flex items-center gap-2 text-primary hover:text-primary/80 font-semibold transition-colors text-sm">
            <ArrowLeft size={18} />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-card border border-border rounded-xl p-4 text-center">
                ...
                <p className="text-2xl font-bold text-primary">{Math.round((c.totalCast / c.totalRegistered) * 100)}%</p>
              </div>
              <div className="bg-card border border-border rounded-xl p-4 text-center">
                <p className="text-xs text-muted-foreground mb-1">गणना प्रगति</p>
                <p className={`text-2xl font-bold ${c.countedPercent === 100 ? 'text-green-600' : 'text-primary'}`}>{c.countedPercent}%</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-xl p-6 mb-8">
              ...
              <div className="flex items-center gap-4 mb-3">
                <div className="flex-1">
                  ...
                  <div className="w-full bg-muted rounded-full h-4">
                    <div className="bg-primary h-4 rounded-full transition-all relative" style={{ width: `${c.countedPercent}%` }}>
              ...
              {c.countedPercent < 100 && (
                <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 mt-3">
                  <p className="text-xs text-muted-foreground">मत गणना जारी छ। अझै {remaining.toLocaleString()} मत गणना बाँकी छ।</p>
                </div>
              )}
            </div>
              ...
              <div className="bg-card rounded-xl p-6 border border-border">
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={c.candidates} layout="vertical" margin={{ left: 20 }}>
                    ...
                    <YAxis type="category" dataKey="name" width={120} tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }} />
                    <Tooltip contentStyle={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px' }} formatter={(value: number) => [value.toLocaleString(), 'मत']} />
                    <Bar dataKey="votes" radius={[0, 6, 6, 0]}>
                      {c.candidates.map((cand, i) => <Cell key={i} fill={cand.color} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      ...
                      cy="50%"
                      labelLine={false}
                      label={({ name, votes }) => `${name.split(' ')[0]} ${Math.round((votes / totalCandVotes) * 100)}%`}
                      outerRadius={100}
                      dataKey="votes"
                      nameKey="name"
                    >
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {c.candidates.map((cand, i) => {
                      ...
                      return (
                        <tr key={i} className={`border-b border-border hover:bg-muted/50 transition-colors ${i === 0 ? 'bg-primary/5' : ''}`}>
                          <td className="px-5 py-3 text-foreground text-sm font-bold">{i === 0 ? 'विजेता' : `#${i + 1}`}</td>
                          <td className="px-5 py-3 font-semibold text-foreground text-sm">{cand.name}</td>
                          <td className="px-5 py-3">
                            <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ backgroundColor: `${cand.color}20`, color: cand.color }}>{cand.party}</span>
                          </td>
                          <td className="px-5 py-3 text-foreground text-sm font-semibold">{cand.votes.toLocaleString()}</td>
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              <div className="w-full bg-border rounded-full h-2 max-w-[120px]">
                                <div className="h-2 rounded-full" style={{ width: `${pct}%`, backgroundColor: cand.color }} />
                              </div>
                              <span className="font-bold text-sm min-w-fit" style={{ color: cand.color }}>{pct}%</span>
                            </div>
                          </td>
                        </tr>
                      );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-3 mb-8">
            <button
              ...
              className={`px-5 py-2.5 rounded-lg font-semibold text-sm transition-all ${
                electionType === 'direct' ? 'bg-primary text-primary-foreground' : 'bg-card border border-border text-foreground hover:border-primary'
              }`}
            >
              प्रत्यक्ष निर्वाचन
            </button>
            <button
              ...
              className={`px-5 py-2.5 rounded-lg font-semibold text-sm transition-all ${
                electionType === 'proportional' ? 'bg-primary text-primary-foreground' : 'bg-card border border-border text-foreground hover:border-primary'
              }`}
            >
              समानुपातिक निर्वाचन
            </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <div className="bg-card rounded-xl p-6 border border-border">
              ...
              <p className="text-xs text-muted-foreground mb-5">कुल {electionType === 'direct' ? '१६५' : '११०'} सिट</p>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={currentNational} margin={{ top: 10, right: 10, left: 0, bottom: 40 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                  <XAxis dataKey="party" angle={-30} textAnchor="end" height={60} tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }} />
                  <YAxis tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }} />
                  <Tooltip contentStyle={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px' }} />
                  <Bar dataKey="seats" radius={[6, 6, 0, 0]}>
                    {currentNational.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-card rounded-xl p-6 border border-border">
              ...
              <p className="text-xs text-muted-foreground mb-5">कुल खसेको मत: {(totalNationalVotes / 1000000).toFixed(1)}M</p>
              <ResponsiveContainer width="100%" height={350}>
                <PieChart>
                  <Pie
          ...
          <div className="bg-card rounded-xl p-6 border border-border mb-8">
            ...
            <div className="mb-3">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm text-muted-foreground">खसेको मत: {(totalVotesCast / 1000000).toFixed(1)}M / {(totalRegisteredVoters / 1000000).toFixed(1)}M</span>
                <span className="text-sm font-bold text-primary">{turnoutPercent}%</span>
              </div>
              <div className="w-full bg-muted rounded-full h-5 relative">
                <div className="bg-primary h-5 rounded-full transition-all flex items-center justify-end pr-2" style={{ width: `${turnoutPercent}%` }}>
                  <span className="text-xs font-bold text-primary-foreground">{turnoutPercent}%</span>
                </div>
              </div>
              <div className="flex items-center justify-between mt-1">
                <span className="text-xs text-muted-foreground">मतदान नगर्ने: {((totalRegisteredVoters - totalVotesCast) / 1000000).toFixed(1)}M</span>
          ...
          <div className="bg-card rounded-xl overflow-hidden border border-border">
            <div className="p-5 bg-primary text-primary-foreground">
              <h3 className="text-lg font-bold">{electionType === 'direct' ? 'प्रत्यक्ष' : 'समानुपातिक'} - दल अनुसार विस्तृत नतिजा</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                      ...
                      <tr key={index} className="border-b border-border hover:bg-muted/50 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-sm min-w-fit" style={{ color: row.color }}>{pct}%</span>
                          </div>
                        </td>
                      </tr>
                    );
            ...
            {displayedResults.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Winner */}
                  <div className="flex items-center gap-3 mb-3 p-3 rounded-lg" style={{ backgroundColor: `${winner.color}10` }}>
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: winner.color, color: '#fff' }}>
                      #१
                    </div>
                    <div className="flex-1">
                      <p className="font-bold text-foreground text-sm">{winner.name}</p>
                      <p className="text-xs" style={{ color: winner.color }}>{winner.party} - {winner.votes.toLocaleString()} मत ({winPct}%)</p>
                    </div>
                  </div>
                    ...
                    {c.candidates.slice(0, 3).map((cand, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 bg-muted rounded-full h-1.5">
                          <div className="h-1.5 rounded-full" style={{ width: `${Math.round((cand.votes / totalCandVotes) * 100)}%`, backgroundColor: cand.color }} />
                        </div>
                      </div>
                    ))}
                  </div>
